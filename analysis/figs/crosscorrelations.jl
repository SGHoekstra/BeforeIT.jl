...
function generate_crosscorrelation_graphs(
    ...
    # Load the real time series data
    if country == "netherlands"
        real_data = Bit.NETHERLANDS_CALIBRATION.data
    else
        # Load from file for other countries
        real_data = matread(("./src/utils/" * "calibration_data/" * country * "/data/1996.mat"))["data"]
    end
    ...
    model_path = "./data/" * country * model_folder * "2016Q1.jld2"
    model = load(model_path)["model_dict"]
    ...
    for i in 1:number_quarters
        ...
        model_path = "./data/" * country * model_folder * "$(year_val)Q$(quarter_val).jld2"
...
generate_crosscorrelation_graphs("netherlands"; empirical_distribution = false, abmx = false, unconditional_forecasts = true)
generate_crosscorrelation_graphs("netherlands"; empirical_distribution = false, abmx = true, unconditional_forecasts = true)