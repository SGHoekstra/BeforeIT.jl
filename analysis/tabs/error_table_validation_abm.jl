...
function error_table_validation_abm(
    ...
    for i in 1:number_quarters
        ...
        for j in 1:number_horizons
            ...
            forecast_quarter_num =
                Bit.date2num(lastdayofmonth(Bit.num2date(quarter_num) + Month(3 * horizon)))
    ...
    h5open(forecast_folder * "/forecast_validation_abm.h5", "w") do file
    ...
    # The VAR model comparison - using original path for now
    ...
    for j in 1:length(horizons)
        ...
        for l in 1:number_variables
            dm_error_validation_abm =
                view(error_validation_abm, :, j, l)[map(!, isnan.(view(error_validation_abm, :, j, l)))]
            dm_error_validation_var =
                view(error_validation_var, :, j, l)[map(!, isnan.(view(error_validation_var, :, j, l)))]
    ...
    open(output_folder * "/rmse_validation_abm.tex", "w") do fid
        for line in latex
            write(fid, line * "\n")
        end
    end
    ...
    for j in 1:length(horizons)
        h = horizons[j]
        for l in 1:number_variables
            mz_forecast = (view(error_validation_abm, :, j, l) + view(actual, :, j, l))[map(
                !,
                isnan.(view(error_validation_abm, :, j, l) + view(actual, :, j, l)),
            )]
    ...
    open(output_folder * "/bias_validation_abm.tex", "w") do fid
        for line in latex
            write(fid, line * "\n")
        end
    end
