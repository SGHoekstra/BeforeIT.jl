# In this tutorial we illustrate how to calibrate the model to the Dutch data for a specific quarter
...
# We start from loading the calibration oject for netherlands, which contains 4 datasets: calibration_data, data, and ea
# These are saved within BeforeIT for the Dutch case, and would need to be appropriately generated for other countries

cal = Bit.NETHERLANDS_CALIBRATION
fieldnames(typeof(cal))

# These are essentually 3 dictionaries with well defined keys, such as
...
parameters, initial_conditions = Bit.get_params_and_initial_conditions_netherlands(cal, calibration_date; scale = 0.0001)
