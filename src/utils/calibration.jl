...
function get_params_and_initial_conditions_netherlands_households_own_firms(calibration_object, calibration_date; scale = 0.0001)
    calibration_data = calibration_object.calibration
    data = calibration_object.data
    ea = calibration_object.ea
    max_calibration_date = calibration_object.max_calibration_date
    ...
    capitalformation_dwellings= zeros(62,1) #calibration_data["capitalformation_dwellings"][:,T_calibration];
    ...
    income_tax = calibration_data["income_tax"][T_calibration]
    capital_taxes = calibration_data["capital_taxes"][T_calibration]
    taxes_products_fixed_capitalformation = calibration_data["taxes_products_capitalformation"][T_calibration]
    taxes_products_capitalformation_dwellings = 0 #calibration_data["taxes_products_capitalformation_dwellings"][T_calibration];
    ...
    wages = compensation_employees * (1 - sum(employers_social_contributions) / sum(compensation_employees)) # Note: owerwrighting the wages variable here!
    household_income_tax = income_tax - corporate_tax
    ...
    w_UB_unscaled=timescale*unemployment_benefits/unemployed;
    sb_inact_unscaled=timescale*pension_benefits/inactive;
    sb_other_unscaled=timescale*(social_benefits+other_net_transfers-unemployment_benefits-pension_benefits)/(sum(employees)+unemployed+inactive);
    ...
    tau_K_s = taxes_production ./ output
    b_CF_g = fixed_capital_formation_other_than_dwellings / sum(fixed_capital_formation_other_than_dwellings)
    b_CFH_g = zeros(62,1)#capitalformation_dwellings / sum(capitalformation_dwellings)
    b_HH_g = household_consumption / sum(household_consumption)
    a_sg = intermediate_consumption ./ sum(intermediate_consumption, dims = 1)
    replace!(a_sg, NaN => 0.0)
    c_G_g = government_consumption / sum(government_consumption)
    ...
    tau_SIW = household_social_contributions / sum(wages)
    tau_EXPORT = sum(taxes_products_export) / sum(exports - reexports)
    tau_CF = 0# sum(taxes_products_capitalformation_dwellings) / sum(capitalformation_dwellings)
    tau_G = sum(taxes_products_government) / sum(government_consumption)
    psi = (sum(household_consumption) + sum(taxes_products_household)) / disposable_income
    psi_H = 0#(sum(capitalformation_dwellings) + sum(taxes_products_capitalformation_dwellings)) / disposable_income
    theta_DIV =
        timescale * (mixed_income + property_income) / (
            sum(
                BeforeIT.pos(
    ...
    E_i = D_I + sum(fixed_assets) - L_I; # total equity of firms including dwellings
    K_H = E_i; # Households own all equity of firms.

    tau_K  = capital_taxes/sum(K_H); 
    # Initial conditions
    Y =
        timescale * sum(output) .* data["real_gdp_quarterly"][T_estimation_exo:T_calibration_exo] ./
        data["real_gdp_quarterly"][T_calibration_exo]
    ...
    # Load the data
    #df = CSV.read("./BeforeIT.jl/src/utils/calibration_data/netherlands/national_index_prices.csv", DataFrame)
    df = CSV.read("./src/utils/calibration_data/netherlands/national_index_prices.csv", DataFrame)
    # Convert to TimeArray and ensure the Date column is DateTime type
    index_prices = TimeArray(df[:, :Date], df[:, :Close])
    index_prices = index_prices[timestamp(index_prices) .< calibration_date]

    # Aggregate to quarterly data
    index_prices = values(percentchange(collapse(index_prices, Dates.quarter, last)))

    ## Calculate the rate of change and handle negative values
    #index_prices[index_prices .< 0] .= 0.0
...
function get_params_and_initial_conditions_netherlands(calibration_object, calibration_date; scale = 0.0001)
    calibration_data = calibration_object.calibration
    data = calibration_object.data
    ea = calibration_object.ea
    max_calibration_date = calibration_object.max_calibration_date
