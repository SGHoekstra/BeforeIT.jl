#=
function get_predictions_from_sims(data, quarter_num, horizon, number_seeds; country = "netherlands", long_run = false)
...
function get_predictions_from_sims(data, quarter_num, horizon, number_seeds; country = "netherlands", abmx = false, longrun = false, empirical_distribution = false, unconditional_forecasts = false)
    ...
    folder = (empirical_distribution ? "/empirical" : "/calibrated") * (abmx ? (unconditional_forecasts ? "/abmx_uf" : "/abmx") : "/abm") * (longrun ? "/long_run" : "/short_run") * "/simulations/"  
    file_name = "data/" * country * folder * string(year(date)) * "Q" * string(quarterofyear(date)) * ".jld2"
    
    println("year: ", year(date))
    println("quarter: ", quarterofyear(date))
    ...
    nominal_sector_gva = permutedims(reshape(sims.nominal_sector_gva,(size(sims.nominal_sector_gva,1),62,Int(size(sims.nominal_sector_gva,2)/62))),(1,3,2))
    model_dict["nominal_nace10_gva_quarterly"] = Array{Float64}(undef, size(nominal_sector_gva, 1), size(nominal_sector_gva, 2), 10)

    quarter_mask = data["quarters_num"]  .== quarter_num

    model_dict["nominal_nace10_gva_quarterly"][:, :, 1] = sum(nominal_sector_gva[:, :, 1:3], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 1] / sum(nominal_sector_gva[1, 1, 1:3])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 2] = sum(nominal_sector_gva[:, :, 4:26], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 2] / sum(nominal_sector_gva[1, 1, 4:26])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 3] = sum(nominal_sector_gva[:, :, 27], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 3] / sum(nominal_sector_gva[1, 1, 27])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 4] = sum(nominal_sector_gva[:, :, 28:36], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 4] / sum(nominal_sector_gva[1, 1, 28:36])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 5] = sum(nominal_sector_gva[:, :, 37:40], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 5] / sum(nominal_sector_gva[1, 1, 37:40])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 6] = sum(nominal_sector_gva[:, :, 41:43], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 6] / sum(nominal_sector_gva[1, 1, 41:43])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 7] = sum(nominal_sector_gva[:, :, 44], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 7] / sum(nominal_sector_gva[1, 1, 44])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 8] = sum(nominal_sector_gva[:, :, 45:53], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 8] / sum(nominal_sector_gva[1, 1, 45:53])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 9] = sum(nominal_sector_gva[:, :, 54:57], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 9] / sum(nominal_sector_gva[1, 1, 54:57])
    model_dict["nominal_nace10_gva_quarterly"][:, :, 10] = sum(nominal_sector_gva[:, :, 58:62], dims=3) *  data["nominal_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 10] / sum(nominal_sector_gva[1, 1, 58:62])
    tmp = model_dict["nominal_nace10_gva_quarterly"][2:13,:,:]

    if !longrun
        if q<4
            model_dict["nominal_nace10_gva"] = Array{Float64}(undef, 3, number_seeds, 10)
        else
            model_dict["nominal_nace10_gva"] = Array{Float64}(undef, 4, number_seeds, 10)
        end
        for sector =1:10
            model_dict["nominal_nace10_gva"][:,:,sector] = [
                fill(data["nominal_nace10_gva"][repeat(data["years_num"] .== year_num,1,10)][sector], number_seeds)'
                BeforeIT.toannual(tmp[:,:,sector][(5 - q):(end - mod(q, 4)), :]')'
                ]
        end
    end

    real_sector_gva = permutedims(reshape(sims.real_sector_gva,(size(sims.real_sector_gva,1),62,Int(size(sims.real_sector_gva,2)/62))),(1,3,2))
    model_dict["real_nace10_gva_quarterly"] = Array{Float64}(undef, size(real_sector_gva, 1), size(real_sector_gva, 2), 10)
    
    # Perform the calculations and assign the results to the dictionary
    model_dict["real_nace10_gva_quarterly"][:, :, 1] = sum(real_sector_gva[:, :, 1:3], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 1] / sum(real_sector_gva[1, 1, 1:3])
    model_dict["real_nace10_gva_quarterly"][:, :, 2] = sum(real_sector_gva[:, :, 4:26], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 2] / sum(real_sector_gva[1, 1, 4:26])
    model_dict["real_nace10_gva_quarterly"][:, :, 3] = sum(real_sector_gva[:, :, 27], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 3] / sum(real_sector_gva[1, 1, 27])
    model_dict["real_nace10_gva_quarterly"][:, :, 4] = sum(real_sector_gva[:, :, 28:36], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 4] / sum(real_sector_gva[1, 1, 28:36])
    model_dict["real_nace10_gva_quarterly"][:, :, 5] = sum(real_sector_gva[:, :, 37:40], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 5] / sum(real_sector_gva[1, 1, 37:40])
    model_dict["real_nace10_gva_quarterly"][:, :, 6] = sum(real_sector_gva[:, :, 41:43], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 6] / sum(real_sector_gva[1, 1, 41:43])
    model_dict["real_nace10_gva_quarterly"][:, :, 7] = sum(real_sector_gva[:, :, 44], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 7] / sum(real_sector_gva[1, 1, 44])
    model_dict["real_nace10_gva_quarterly"][:, :, 8] = sum(real_sector_gva[:, :, 45:53], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 8] / sum(real_sector_gva[1, 1, 45:53])
    model_dict["real_nace10_gva_quarterly"][:, :, 9] = sum(real_sector_gva[:, :, 54:57], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 9] / sum(real_sector_gva[1, 1, 54:57])
    model_dict["real_nace10_gva_quarterly"][:, :, 10] = sum(real_sector_gva[:, :, 58:62], dims=3) *  data["real_nace10_gva_quarterly"][repeat(quarter_mask,1,10)][ 10] / sum(real_sector_gva[1, 1, 58:62])
    tmp = model_dict["real_nace10_gva_quarterly"][2:13,:,:]

    if !longrun
        if q<4
            model_dict["real_nace10_gva"] = Array{Float64}(undef, 3, number_seeds, 10)
        else
            model_dict["real_nace10_gva"] = Array{Float64}(undef, 4, number_seeds, 10)
        end
        for sector =1:10
            model_dict["real_nace10_gva"][:,:,sector] = [
                fill(data["real_nace10_gva"][repeat(data["years_num"] .== year_num,1,10)][sector], number_seeds)'
                BeforeIT.toannual(tmp[:,:,sector][(5 - q):(end - mod(q, 4)), :]')'
                ]
        end
    end
    ...
    folder = (empirical_distribution ? "/empirical" : "/calibrated") * (abmx ? (unconditional_forecasts ? "/abmx_uf" : "/abmx") : "/abm") * (longrun ? "/long_run" : "/short_run") * "/predictions/"  
    file_name = "data/" * country * folder * string(year(date)) * "Q" * string(quarterofyear(date)) * ".jld2"

    save(file_name, "model_dict", model_dict)
