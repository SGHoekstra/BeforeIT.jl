...
# Read server URLs from config file if available
server_urls = None
config_file = os.environ.get('CONFIG_FILE', 'beforeit_config.json')
if os.path.exists(config_file):
    ...
    with open(config_file, 'r') as f:
        ...
        if 'server_urls' in config:
            ...
            print(f"Loaded {len(server_urls)} server URLs from config file")
...
# Initialize the ServerMonitor with HTTP URLs
if server_urls:
    monitor = ServerMonitor(server_urls=server_urls)
else:
    # Default to localhost servers on ports 8080-8083
...
# Run calibration
print(f"Starting calibration with {5} iterations...")
